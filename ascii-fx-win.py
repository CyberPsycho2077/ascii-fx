...
import os, sys, time, math, argparse, subprocess, json, platform
...
def get_fastfetch():
    ...
    except: return Text("[fastfetch unavailable on Windows]")

def animate(img, style, bw, override):
    console, t = Console(), 0
    fastfetch = get_fastfetch()
    with Live(console=console, refresh_per_second=20) as live:
        while not msvcrt.kbhit():
            t += 0.2
            ascii_img = render_ascii(img, style, bw, t, True, override)
            live.update(Columns([ascii_img, fastfetch], equal=False, expand=True))
            time.sleep(0.05)
    console.clear(); print("[Exited animation 👋]")

def render_static(img, style, bw, override):
    Console().print(Columns([render_ascii(img, style, bw, override=override), get_fastfetch()], equal=False, expand=True))

def main():
    p = argparse.ArgumentParser(description="ascii-fx (Windows)")
...
if __name__ == "__main__":
    if platform.system() != "Windows":
        sys.exit("❌ This version is intended for Windows only.")

    import msvcrt  # ✅ moved inside Windows check
    main()
